...
export default function Home({ navigation }) {
  ...
  // const [history2, setHistory2] = useState<Result[]>([]);
  ...
  const reset = () => {
    setDiceAmount(1);
    setDiceValue(0);
    setDiceGroup((prevCount) =>
      prevCount.map((count) => ({ ...count, counter: 0 }))
    );
  };
          ...
          {diceValue === 0 ? (
            ...
            <View style={{ height: height * 0.13 }}>
              ...
              <View style={styles.controlsResult}>
                <Pressable
                  children={<Minus />}
                  onPress={() => setDiceAmount(diceAmount - 1)}
                  disabled={diceAmount === 1}
                />
                {/* RESULT */}
                <Text
                  children={result.sum}
                  style={{ fontSize: 58, fontWeight: "bold" }}
                />
                <Pressable
                  children={<Plus />}
                  onPress={() => setDiceAmount(diceAmount + 1)}
              ...
              {diceAmount > 1 && (
                <Text
                  children={result.numbers.toString()}
                  style={{
                    alignSelf: "center",
                    color: "#787878",
                    fontSize: 18,
      ...
      <View style={styles.footer}>
        <View style={styles.groupButtons}>
          {diceGroup.slice(0, 4).map((dice) => (
            <DiceButton
              key={dice.number}
              counter={dice.counter.toString()}
              diceNumber={dice.number}
              diceFace={dice.diceFormat}
              action={() => buttonAction(dice.number, diceAmount, dice.id)}
            />
          ))}
        </View>
        <View style={styles.groupButtons}>
